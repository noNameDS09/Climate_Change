...
const Home = () => {
    ...
    return (
        <div className="flex flex-col gap-y-5 justify-center items-center h-[90vh] mt-24 mb-20" style={{ backgroundColor: 'rgba(10, 10, 11, 0)' }}>
            <div className="flex flex-col justify-center text-center items-center w-[70rem] gap-y-3">
                <h1 className={`text-zinc-300 text-2xl ${roboto.className}`}>Introducing our Climate Change Awareness Chatbot</h1>
            ...
            <div className="w-full max-w-lg bg-emerald-800 rounded-lg shadow-lg p-4 flex flex-col h-4/5">
                <div className="flex-grow overflow-y-auto space-y-4 px-4">
                    {messages.map((msg, index) => (
                        <div key={index} className={`p-3 rounded-lg max-w-[75%] w-fit ${msg.sender === 'user' ? 'bg-green-400/[0.6] ml-auto text-right' : 'bg-gray-400 mr-auto text-left'}`}>
                            <span>{msg.text}</span>
                        </div>
                    ))}
                    {loading && (
                ...
                <div className="flex items-center mt-4 space-x-4 px-4">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 w-18 min-w-18 text-white rounded-md focus:outline-none ${arrow ? 'hover:bg-green-600 duration-300' : 'bg-green-500'}`}
                    >
                        Send
                    </button>
                </div>
